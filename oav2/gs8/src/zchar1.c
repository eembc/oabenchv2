...
/* $Id: zchar1.c 6651 2006-03-13 16:18:19Z stefan $ */
...
int ee_init_cxs(gs_type1exec_state *pcxs) 
{
	gs_rect zrect={{0,0},{0,0}};
    pcxs->i_ctx_p=NULL;		/* so push/pop can access o-stack */
	pcxs->sbw[0]=pcxs->sbw[1]=pcxs->sbw[2]=pcxs->sbw[3]=0.0;
    pcxs->present=0;
	pcxs->char_bbox=zrect;
    pcxs->use_FontBBox_as_Metrics2=0;
    pcxs->num_args=0;
	return 0;
}
...
charstring_execchar(i_ctx_t *i_ctx_p, int font_type_mask)
{
    ...
    const gs_type1_data *pdata=NULL;
...
inline int rect_within_func(gs_rect inner,gs_rect outer) {
	if ((inner).q.y > (outer).q.y)
		return false;
	if ((inner).q.x > (outer).q.x)
		return false;
	if ((inner).p.y < (outer).p.y)
		return false;
	if ((inner).p.x < (outer).p.x)
		return false;
	return true;
}
...
bbox_draw(i_ctx_t *i_ctx_p, int (*draw)(gs_state *), op_proc_t *exec_cont)
{
    ...
    if (rect_within_func(bbox, pbfont->FontBBox))	/* within bounds */
		return nobbox_draw(i_ctx_p, draw);
