...
/* $Id: gsline.c 6651 2006-03-13 16:18:19Z stefan $ */
...
gs_setdotorientation(gs_state *pgs)
{
	...
	/** ECL Roundoff problems in newlib or simulator FP. */
	th_printf("\nFailure: Roundoff error in pgs->ctm see update_ctm. %s : %d\n",__FILE__,__LINE__);
	th_printf("\nxxyy: %e, %e fabs(xxyy): %e, %e\n",
		pgs->ctm.xx,pgs->ctm.yy,fabs(pgs->ctm.xx),fabs(pgs->ctm.yy));
	pgs->ctm.xx=pgs->ctm.yy=0.0;
	th_printf("xyyx: %e, %e fabs(xyyx): %e, %e\n",
		pgs->ctm.xy,pgs->ctm.yx,fabs(pgs->ctm.xy),fabs(pgs->ctm.yx));
	pgs->ctm.xy=pgs->ctm.yx=0.0;
	th_printf("txty: %e, %e fabs(txty): %e, %e\n",
		pgs->ctm.tx,pgs->ctm.ty,fabs(pgs->ctm.tx),fabs(pgs->ctm.ty));
	pgs->ctm.tx=pgs->ctm.ty=0.0;

	/** The numbers come in randomly when this is broken, so you
	must either allow or dissallow the warning. 
	*/
	th_printf("\n*** Possible Tool chain porting issue or compiler option ***\n\n");
	return gs_currentmatrix(pgs, &pgs_lp->dot_orientation);
		
	/** ECL return_error(gs_error_rangecheck); */
}
