...
/* $Id: zfile.c 6651 2006-03-13 16:18:19Z stefan $ */
...
/** ECL gx_device members cannot be redirected globally. Manually defined for this file. */
#undef	fclose
#undef	fopen
#undef	close
...
extern const char iodev_dtype_stdio_zio[];
...
zfile(i_ctx_t *i_ctx_p)
{
	 ...
	 * more information.
	 */
    if (pname.iodev && pname.iodev->dtype == iodev_dtype_stdio_zio) {
...
{   /* i_ctx_p is NULL running arg (@) files. 
...
#if (BMDEBUG==1)
	{ /*SG: DEBUG */
		char *buf=(char *)malloc((1+flen)*sizeof(char));
		char dbgname[32]=".ps";
		strncpy(buf,fname,flen);
		buf[flen]=0;
		if (strstr(buf,dbgname)!=NULL) {
			flen=flen;
		} 
		th_printf("Opening file %s\n",buf);
		free(buf);
	}
#endif
    strcat(fmode, gp_fmode_binary_suffix);
	...
	if (gp_file_name_reduce(fname, flen, buffer, &blen1) != gp_combine_success)
	    goto skip;
	/* SG we know our resources are all in the rom FS under %rom%/Resource  */
	/* Make sure we get it directly from romfs rather then FS				*/
	if (strstr(buffer,"Resource/")!=NULL) {
		char mybuf[256];
		int code;
		/* set iodev details */
	    gx_io_device *iodev=gs_findiodevice("%rom%",4);
		/* load the file from ROMFS */
		code = iodev->procs.open_file(iodev, buffer, blen1, fmode,
						  &s, (gs_memory_t *)mem);
		if (code < 0)
			return_error(e_invalidfileaccess);
		memcpy(mybuf, "%rom%", 5);
		memcpy(mybuf+5, buffer,blen1);
		memcpy(buffer,mybuf,blen1+5);
		*pclen = blen1+5;
		make_stream_file(pfile, s, "r");
#if (BMDEBUG==1)
		th_printf("\topened from rom\n");
#endif
	...
	for (pi = 0; pi < r_size(&pfpath->list); ++pi) {
		...
		/* SG: skip curdir, enforce real path */
		if (gp_file_name_is_current(pstr,plen))
			continue;
		...
		th_printf("\topened from rom\n");
			...
			if (iodev_os_open_file(iodev_default, (const char *)buffer, blen1, (const char *)fmode,
				...
				iodev->procs.fclose(iodev_default, (ee_FILE *)&s);
				pfile = NULL;
				return_error(e_invalidfileaccess);
			}
			pfile = NULL; /* safety */
