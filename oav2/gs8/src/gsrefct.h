...
/* $Id: gsrefct.h 6651 2006-03-13 16:18:19Z stefan $ */
...
/* ETC: Make sure the "free" macro is not invoked, since it redirects to th_free */
/* change rc.free() to rc.rc_free ( */

#define rc_free_struct(vp, cname)\
  BEGIN\
    IF_RC_DEBUG(rc_trace_free_struct(vp, &(vp)->rc, cname));\
    (vp)->rc.rc_free ((vp)->rc.memory, (void *)(vp), cname);\
  END
