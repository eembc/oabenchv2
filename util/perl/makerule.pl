#!/usr/bin/perl
#==============================================================================
#$RCSfile: makerule.pl,v $
#
#   DESC : Make File rule generator
#
...
#    CVS : $Revision: 1.30 $
#
# NOTE   : Documentation in util/doc/makerule.html
#
#------------------------------------------------------------------------------
...
makerule <switches> <file specs>
    ...
    -cq-                  Disables quoting.
	-cs                   Enables generation of source file for rules without $<
    ...
    -occ                  Put all source files in one compile command. NOTE: The
                          -o option overrides this setting.
    ...
    -tt <targetname>      Sets the target name for generated files.
...
my $version = '$Revision: 1.30 $ $Date: 2005/11/15 00:42:58 $';
...
$targetname			= "targets";	# -tt assume targets
...
$one_compile_command = 0;           # all source files in one compile command
...
ARGS: while ( $e=shift( @ARGV ) )
    ...
    if ( $e =~ /^-/ )  # is it a command line switch??
       ...
       #
          ...
          if ( $1 )
             { $targetname = $1 }
          elsif ( $targetname = shift( @ARGV ) )
             {
             }
          else
             {
             print STDERR "\nERROR: No target name after -tt switch!\n";
             usage();
             exit 2;
             }
          }
...
# add new -t options onto previous elsif's, or they become targets
          ...
          @all_inc = ();  # empty the all-include list
          @all_src = ();  # empty the all-source list
          }
       # END -T OPTIONS
       ...
       if ( $e =~ /^-ox$/i )
          ...
          { 
          $one_compile_command = 1;  # all source files in one compile command
          print STDERR "ALL SOURCE FILES IN ONE COMPILE COMMAND\n" #if $verbose;
          }
       elsif ( $e =~ /^-o(.*)/i )
          {
          if ( $1 )
       ...
       if ( -f $e )
       { 
		   #SG: allow a list of files to be included
		   if ($e =~ /.inc$/) {
				if (open(INCFILES,"<$e")) {
					while (<INCFILES>) {
						chomp;
						next if (/^\s*\#/); #skip comments, anything else should be a filename, one per line.
						push @files, $_;
					}
					close($e);
				} else {
					print "Could not open $e to process file list!\n";
					next;
				}
		   } else {
				@files = ( $e ) 
...
#rlf - Move this at the end of the include file list.
       foreach $e (@otargets) {
             if ( ($line_len + length(" $e")) >= $margin-1 )
                {
                print OFILE ' ' x (($margin - $line_len) - 2) . " \\\n";
...
# dumping executable targets
   if ( (@ctobjs, @ccobjs) == 0 )
      ...
      if ($one_compile_command) {
         my $utarget_src = $utarget . "_SRC";
         print OFILE "$utarget_src = ";
         foreach $e (@all_src) {print OFILE "\\\n    $e ";}
         print OFILE "\n\n";
         
         my $utarget_dep = $utarget . "_DEP";
         print OFILE "$utarget_dep = \$\($utarget_src\) ";
         foreach $e (@all_inc, @otargets) {print OFILE "\\\n    $e ";}
         print OFILE "\n\n";
         ...
         print OFILE "$tobj_prefix$ctarget$ttgt_ext:  \$\($utarget_dep\) ";
         foreach $e (@atargets) {print OFILE "$e ";}
         print OFILE "\n";
         if ( $recipe ) {
            print OFILE "\t$recipe ";
            # GHS Specific, fix for GCC
            print OFILE "-o$obj_prefix$ctarget$fn$tgt_ext \$\($utarget_src\)\n";
         }
      } else {
	  print OFILE "$utarget = ";
	  foreach $e (@ctobjs) {print OFILE "\\\n    $e ";}
      ...
      if ($one_compile_command) {
         print OFILE " $obj_prefix$ctarget$fn$tgt_ext ";
      } else {
...
Switches:
    ...
    -occ                  Put all source files in one compile command.
    -r <recipe>           Sets the recipe to be placed after each rule.
    -rx                   Clears the current recipe.
