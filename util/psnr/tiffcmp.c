/* $Header: /cvs/eembc2/techtag/util/psnr/tiffcmp.c,v 1.3 2005/11/17 01:06:23 rick Exp $ */
/**
 * @file
 * @brief Compare two TIFF files
 *
 * Tiffcmp  compares  the  tags and data in two files created according to
 * the Tagged Image File Format, Revision 6.0.  The schemes used for 
 * compressing  data  in each file are immaterial when data are compared-data
 ...
 * equal.  By default, tiffcmp will terminate if it encounters any 
 * difference.
 *
 * @verbatim
...
tiffcmp(TIFF* tif1, TIFF* tif2)
{
	...
	buf1 = (unsigned char *)malloc(size1 = TIFFScanlineSize(tif1));
	buf2 = (unsigned char *)malloc(TIFFScanlineSize(tif2));
